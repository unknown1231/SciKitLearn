...
for dataset in combine:
    dataset['Title'] = dataset.Name.str.extract('([A-Za-z]+)\.', expand=False)# extracts the titles(mr,mrs etc) from the names feature
pd.crosstab(train_data['Title'], train_data['Sex'])# this probably gives the count of the titles and puts them in male female category
# eg: Mr 0 517
...
if ask == '1':
    ...
    print('LogisticRegression accuracy is:', accuracy_log * 100)
...
if ask == '2':
    ...
    print('Support Vector Machine accuracy is:', accuracy_svc * 100)

if ask == '3':
    perceptron_train = Perceptron()
    perceptron_train.fit(x_train, y_train)
    perceptron_predict = perceptron_train.predict(x_test)
    accuracy_perceptron = perceptron_train.score(x_train, y_train)
    print('Perceptron accuracy is:', accuracy_perceptron * 100)

if ask == '4':
    gaussian = GaussianNB()
    gaussian.fit(x_train, y_train)
    gaussian_predict = gaussian.predict(x_test)
    accuracy_nb = gaussian.score(x_train, y_train)
    print('Naive Bayes accuracy is:',  accuracy_nb * 100)

if ask == '5':
    decision_tree = DecisionTreeClassifier()
    decision_tree.fit(x_train, y_train)
    tree_predict = decision_tree.predict(x_test)
    accuracy_dt = decision_tree.score(x_train, y_train)
...
if ask == '6':
    ...
    accuracy_rf = random_forest.score(x_train, y_train)
...
if ask == '7':
    ...
    print('k-NearestNeighbours accuracy is:', accuracy_knn * 100)

# table to show all models and their accuracies
# flaw in the below code....if i choose any of the above options...only that option is executed...so the entire list below cannot be executed!!
# models = pd.DataFrame({'Model': ['Logistic Regression', 'Linear SVC', 'Perceptron',
#                                  'Naive Bayes', 'Decision Tree', 'Random Forest', 'KNN'],
#                        'Accuracy': [accuracy_log, accuracy_svc, accuracy_perceptron, accuracy_nb, accuracy_dt,
#                                     accuracy_rf, accuracy_knn]})
#
# print('\n', '-' * 50, '\n\t\t\t\tMODEL ACCURACIES\t\t\t\t', '\n', '-' * 50)
